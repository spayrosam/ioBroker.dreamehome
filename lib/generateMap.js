module.exports = async function DH_GenerateMap(DH_CurMap,
  DH_Did,
  DH_Map,
  UserLang,
  fullQuality,
  ...
  try {
    ...
    const elements = {
      ...
      carpets: typeof currentMap.carpet_info === 'object' ? currentMap.carpet_info :
        typeof currentMap.carpet_polygon === 'object' ? currentMap.carpet_polygon : {},
    ...
    // Assign carpet colors to SVG
    let carpetColorIndex = 0;
    Object.entries(elements.carpets).forEach(([id, rect]) => {
      const color = ColorsCarpet[carpetColorIndex % ColorsCarpet.length];
    ...
    const html = `<!DOCTYPE html><html><head>
            ...
            <style>
                ...
                .carpet.selected path {
                    filter: drop-shadow(0 0 2px lime);
                    animation: pulse-carpet 2s infinite;
                }

                @keyframes pulse-carpet {
                    0% { stroke-width: 1px; opacity: 0.8; }
                    50% { stroke-width: 1.5px; opacity: 1; }
                    100% { stroke-width: 1px; opacity: 0.8; }
                }
                ...
                .dining-table {
					...
					pointer-events: none; /* Clicks go through the table */
                }
				...
				.selection-summary span[id$="-summary"] {
					font-weight: bold;
                }
    ...
    const getroomname = PrepareRoomsMappings(room.room_id);
					...
					${Object.entries(elements.carpets).map(([id, rect]) => {
    					...
    					// Check if it's the new polygon structure: [polygonPoints, material, flags]
    					if (Array.isArray(rect) && rect.length === 3 && Array.isArray(rect[0])) {
    					  // New structure: polygon points
    					  const [polygonPoints, material, flags] = rect;

    					  // Transform all polygon points
    					  const points = [];
    					  for (let i = 0; i < polygonPoints.length; i += 2) {
     					   const [tx, ty] = toCanvas(polygonPoints[i], polygonPoints[i + 1]);
     					   points.push(`${tx},${ty}`);
     					 }
  					    ...
  					    return `
                            <svg class="carpet" id="carpet-${id}" style="position:absolute;top:0;left:0;width:100%;height:100%">
                                <path d="${pathData}" fill="${color}33" stroke="${color}" stroke-width="1.5"/>
                            </svg>`;
  					  } else {
  					    // Old structure: rectangle coordinates [x1, y1, x2, y2, materials, areas]
  					    ...
  					    const pathData = `M${tx1},${ty1} L${tx2},${ty2} L${tx3},${ty3} L${tx4},${ty4} Z`;
					...
					${Array.isArray(wallsInfo?.doors) ? wallsInfo.doors.map(door => {
					...
					${(() => {
    					const furnitureInfo = currentMap.funiture_info || [];
    					let zoneHtml = '';

    					furnitureInfo.forEach((furniture, index) => {
    					  if (Array.isArray(furniture) && furniture.length >= 9) {
    					    ...
    					    if ([6].includes(type) && width >= 600 && width <= 2200 && height >= 600 && height <= 1000) {
    					      ...
    					      // Original padding from the code: 10 cm buffer
    					      const zonePadding = 100;
    					      const zoneWidth = width + zonePadding * 2;
    					      const zoneHeight = height + zonePadding * 2;

    					      // Show the actual table
        					  const tableCenterX = toCanvas(posX, posY)[0];
    					      const tableCenterY = toCanvas(posX, posY)[1];
    					      const tableWidth = width * scale;
    					      const tableHeight = height * scale;

    					      zoneHtml += `
									<div class="dining-table"
       					              style="position:absolute; width:${tableWidth}px; height:${tableHeight}px;
                   					  left:${tableCenterX - tableWidth/2}px;
            					      top:${tableCenterY - tableHeight/2}px;
            					      background:rgba(139,69,19,0.8); border:2px solid #8B4513;
            					      border-radius:4px; transform:rotate(${angle}deg);
                ...
                <div class="control-panel">
                    ...
                    <div class="menu-level" id="main-menu">
                        <div class="menu-title" id="mainMenuLabel">Main Menu</div>
                        <button class="menu-item" id="viewSettingsLabel" data-next="view-menu">
                            <i>
							...
							<span class="menu-text">View settings</span>
                        </button>

						<button class="menu-item" id="cleaningModeLabel" data-next="clean-menu">
							<span class="menu-label">Cleaning Mode</span>
						...
						<button class="menu-item" id="spectacleLabel" data-next="spectacle-menu">
                            <i>&#128171;</i>Spectacle
                        </button>
                    ...
                    <div class="menu-level" id="view-menu">
						...
						<div class="colorinput-container">
                            <label for="bg-color-picker" id="backgroundLabel">Background:</label>
                        ...
                        <button class="back-button" id="backViewLabel" data-back="main-menu">&#128072; Back</button>
                    </div>

					<!-- Cleaning Menu Level 2 -->
					<div class="menu-level" id="clean-menu">
					    <div class="menu-title" id="cleaningMenuLabel">Cleaning Menu</div>
					    ...
					    <button class="back-button" id="backCleaningLabel" data-back="main-menu">&#128072; Back</button>
					</div>

					<!-- Spectacle Menu Level 2 -->
                    <div class="menu-level" id="spectacle-menu">
                        <div class="menu-title" id="spectacleMenuLabel">Spectacle Menu</div>
						<div class="slider-container">
                            <label for="animation-speed" id="speedLabel">Speed:</label>
                        ...
                        <button class="back-button" id="backSpectacleLabel" data-back="main-menu">&#128072; Back</button>
                    </div>
                ...
                <div id="summary-tooltip" class="summary-tooltip">
                    <div class="selection-summary">
					    <div><strong id="summaryTitleLabel">Current Selection:</strong></div>
                        <div><span id="summaryModeLabel">Mode:</span> <span id="mode-summary">-</span></div>
                        <div><span id="summarySuctionLabel">Suction:</span> <span id="suction-summary">-</span></div>
                        <div><span id="summaryWaterLabel">Water:</span> <span id="water-summary">-</span></div>
                        <div><span id="summaryRouteLabel">Route:</span> <span id="route-summary">-</span></div>
                        <div><span id="summaryRepeatLabel">Repeat:</span> <span id="repeat-summary">-</span></div>
                    </div>
                </div>
                ...
                <script>
                    ...
                    const translations = {
                        'EN': {
                            ...
                            'xRotationLabel': 'X-Rotation',
                            'yRotationLabel': 'Y-Rotation',
                            'zRotationLabel': 'Z-Rotation',
                            ...
                            'backViewLabel': 'Back',
                            'cleaningModeLabel': 'Cleaning Mode',
                            'cleaningMenuLabel': 'Cleaning Menu',
                            'suctionLabel': 'Suction',
                            'waterLabel': 'Water',
                            'routeLabel': 'Route',
                            'repeatLabel': 'Repeat',
                            'backCleaningLabel': 'Back',
                            'spectacleLabel': 'Spectacle',
                            'spectacleMenuLabel': 'Spectacle Menu',
                            'speedLabel': 'Speed',
                            'startMapSpectacleLabel': 'Start Map Spectacle',
                            'stopMapSpectacleLabel': 'Stop Map Spectacle',
                            'startBaseSpectacleLabel': 'Start Base Spectacle',
                            'stopBaseSpectacleLabel': 'Stop Base Spectacle',
                            'backSpectacleLabel': 'Back',

                            // Cleaning Mode
                            'Sweeping': 'Sweeping',
                            'Mopping': 'Mopping',
                            'Sweeping and mopping': 'Sweeping and mopping',
                            'Mopping after sweeping': 'Mopping after sweeping',
                            'Custom room cleaning': 'Custom room cleaning',
							'Not selected': 'Not selected',

                            // Summary Tooltip
                            'summaryTitleLabel': 'Current Selection',
                            'summaryModeLabel': 'Mode',
                            'summarySuctionLabel': 'Suction',
                            'summaryWaterLabel': 'Water',
                            'summaryRouteLabel': 'Route',
                            'summaryRepeatLabel': 'Repeat',
                            'summaryNotSet': '-',
                            ...
                            'summaryMaxPlus': 'Max+',
                            'summaryLow': 'Low',
                            'summaryMiddle': 'Medium',
                            'summaryHigh': 'High',
                            'summaryUltra': 'Ultra',
                            'summaryQuick': 'Quick',
                            'summaryIntensive': 'Intensive',
                            'summaryDeep': 'Deep',
                            'summary1': '1',
                            'summary2': '2',
                            'summary3': '3',
                        ...
                        'DE': {
                            ...
                            'viewSettingsLabel': 'Ansichtseinstellungen',
                            'viewMenuLabel': 'Ansichtsmenü',
                            'xRotationLabel': 'X-Rotation',
                            'yRotationLabel': 'Y-Rotation',
                            'zRotationLabel': 'Z-Rotation',
                            ...
                            'backViewLabel': 'Zurück',
                            'cleaningModeLabel': 'Reinigungsmodus',
                            'cleaningMenuLabel': 'Reinigungsmenü',
                            'suctionLabel': 'Saugstärke',
                            'waterLabel': 'Wassermenge',
                            'routeLabel': 'Reinigungsroute',
                            'repeatLabel': 'Wiederholungen',
                            'backCleaningLabel': 'Zurück',
                            'spectacleLabel': 'Spektakel',
                            'spectacleMenuLabel': 'Spektakel-Menü',
                            'speedLabel': 'Tempo',
                            'startMapSpectacleLabel': 'Karten-Spektakel starten',
                            'stopMapSpectacleLabel': 'Karten-Spektakel stoppen',
                            'startBaseSpectacleLabel': 'Basis-Spektakel starten',
                            'stopBaseSpectacleLabel': 'Basis-Spektakel stoppen',
                            'backSpectacleLabel': 'Zurück',

                            // Cleaning Mode
                            'Sweeping': 'Staubsaugen',
                            'Mopping': 'Wischen',
                            'Sweeping and mopping': 'Saugen und Wischen',
                            'Mopping after sweeping': 'Wischen nach dem Saugen',
                            'Custom room cleaning': 'Raumreinigung anpassen',
                            'Not selected': 'Nicht ausgewählt',

                            // Summary Tooltip
                            'summaryTitleLabel': 'Aktuelle Auswahl',
                            'summaryModeLabel': 'Modus',
                            'summarySuctionLabel': 'Saugstärke',
                            'summaryWaterLabel': 'Wassermenge',
                            'summaryRouteLabel': 'Reinigungsroute',
                            'summaryRepeatLabel': 'Wiederholungen',
                            'summaryNotSet': '-',
                            ...
                            'summaryMaxPlus': 'Max+',
                            'summaryLow': 'Gering',
                            'summaryMiddle': 'Mittel',
                            'summaryHigh': 'Hoch',
                            'summaryUltra': 'Ultra',
                            'summaryQuick': 'Schnell',
                            'summaryIntensive': 'Intensiv',
                            'summaryDeep': 'Tiefenreinigung',
                            'summary1': '1',
                            'summary2': '2',
                            'summary3': '3',
					...
					// Main Menu
					document.getElementById('mainMenuLabel').textContent = translations[UserLang].mainMenuLabel;

					// View Menu
					document.querySelector('#viewSettingsLabel .menu-text').textContent = translations[UserLang].viewSettingsLabel;
					document.getElementById('viewMenuLabel').textContent = translations[UserLang].viewMenuLabel;
					document.getElementById('xRotationLabel').textContent = translations[UserLang].xRotationLabel;
					document.getElementById('yRotationLabel').textContent = translations[UserLang].yRotationLabel;
					document.getElementById('zRotationLabel').textContent = translations[UserLang].zRotationLabel;
					document.getElementById('perspectiveLabel').textContent = translations[UserLang].perspectiveLabel;
					document.getElementById('zoomLabel').textContent = translations[UserLang].zoomLabel;
					document.getElementById('horizontalLabel').textContent = translations[UserLang].horizontalLabel;
					document.getElementById('verticalLabel').textContent = translations[UserLang].verticalLabel;
					document.getElementById('mapSizeLabel').textContent = translations[UserLang].mapSizeLabel;
					document.getElementById('backgroundLabel').textContent = translations[UserLang].backgroundLabel;
					document.getElementById('reset-view-btn').textContent = translations[UserLang].resetViewLabel;
					document.getElementById('backViewLabel').innerHTML = '&#128072; ' + translations[UserLang].backViewLabel;

					// Cleaning Menu
					document.querySelector('#cleaningModeLabel .menu-label').textContent = translations[UserLang].cleaningModeLabel;
					document.getElementById('cleaningMenuLabel').textContent = translations[UserLang].cleaningMenuLabel;
					document.getElementById('backCleaningLabel').innerHTML = '&#128072; ' + translations[UserLang].backCleaningLabel;

					// Spectacle Menu
					document.getElementById('spectacleLabel').innerHTML = '&#128171; ' + translations[UserLang].spectacleLabel;
					document.getElementById('spectacleMenuLabel').textContent = translations[UserLang].spectacleMenuLabel;
					document.getElementById('speedLabel').textContent = translations[UserLang].speedLabel;
					document.getElementById('animation-toggle').textContent = translations[UserLang].startMapSpectacleLabel;
					document.getElementById('Base-station-toggle').textContent = translations[UserLang].startBaseSpectacleLabel;
					document.getElementById('backSpectacleLabel').innerHTML = '&#128072; ' + translations[UserLang].backSpectacleLabel;

					// Summary Tooltip
					document.getElementById('summaryTitleLabel').textContent = translations[UserLang].summaryTitleLabel + ": ";
					document.getElementById('summaryModeLabel').textContent = translations[UserLang].summaryModeLabel + ": ";
					document.getElementById('summarySuctionLabel').textContent = translations[UserLang].summarySuctionLabel + ": ";
					document.getElementById('summaryWaterLabel').textContent = translations[UserLang].summaryWaterLabel + ": ";
					document.getElementById('summaryRouteLabel').textContent = translations[UserLang].summaryRouteLabel + ": ";
					document.getElementById('summaryRepeatLabel').textContent = translations[UserLang].summaryRepeatLabel + ": ";
					...
					const VisCleaningModes = {
					    Sweeping: {
					        ...
					        activeModes: ["SuctionQuiet", "SuctionStandard", "SuctionTurbo", "SuctionMax", "SuctionMaxPlus", "RouteQuick", "RouteStandard", "Repeat1", "Repeat2", "Repeat3"],
					    ...
					    SweepingAndMopping: {
					        ...
					        activeModes: ["SuctionQuiet", "SuctionStandard", "SuctionTurbo", "SuctionMax", "WaterLow", "WaterMiddle", "WaterHigh", "WaterUltra", "RouteQuick", "RouteStandard", "Repeat1", "Repeat2", "Repeat3"],
					    ...
					    MoppingAfterSweeping: {
					        ...
					        options: ['Suction', 'Water', 'Route', 'Repeat'],
					        activeModes: ["SuctionQuiet", "SuctionStandard", "SuctionTurbo", "SuctionMax", "SuctionMaxPlus", "WaterLow", "WaterMiddle", "WaterHigh", "WaterUltra", "RouteQuick", "RouteStandard", "RouteIntensive", "RouteDeep", "Repeat1", "Repeat2", "Repeat3"],
                    ...
                    const VisCleaningOptions = {
                        Suction: [
                            { value: "Quiet", label: "Quiet", svg: \`<svg class="mode-option-buttons-svg" width="40" height="40" style="fill-rule:evenodd" viewBox="0 0 40 40">
                    ...
                    // Device State Mapping and Configuration
                    // =============================================

                    // Mapping between UI values and device state values
                    const DeviceStateMapping = {
                        CleaningMode: {
                            // English
                            CustomRoomCleaning: 1,
                            SweepingAndMopping: 5120,
                            Mopping: 5121,
                            Sweeping: 5122,
                            MoppingAfterSweeping: 5123,
                            // German
                            'Raumreinigung anpassen': 1,
                            'Saugen und Wischen': 5120,
                            'Wischen': 5121,
                            'Staubsaugen': 5122,
                            'Wischen nach dem Saugen': 5123
                        },
                        WaterVolume: {
                            Low: 1, Niedrig: 1,
                            Middle: 2, Mittel: 2,
                    ...
                    // Device state IDs
                    const DEVICE_STATES = {
                        mode: "${prefix}.control.CleaningMode",
                        suction: "${prefix}.control.SuctionLevel",
                        maxPlus: "${prefix}.control.SuctionMaxPlus",
                        water: "${prefix}.control.WaterVolume",
                        route: "${prefix}.control.CleanRoute",
                        repeat: "${prefix}.control.Repeat"
                    };

                    // Global flag to prevent sync loops
                    let isSyncing = false;

                    // Core Functions with Bidirectional Sync
                    // =============================================

                    // Converts UI values to device-compatible values
                    function getDeviceValue(optionType, uiValue) {
                        const mapping = DeviceStateMapping[optionType];
                        if (!mapping) {
                            console.warn('No mapping for ' + optionType);
                            return null;
                        }

                        // Case-insensitive search with fallback
                        const foundKey = Object.keys(mapping).find(
                            key => key.toLowerCase() === uiValue.toLowerCase()
                        );
                        return foundKey ? mapping[foundKey] : null;
                    }

                    // Converts device values back to UI values
                    async function getUIValue(optionType, deviceValue) {
                        const mappingLookup = {
                            mode: 'CleaningMode',
                            ...
                            route: 'CleanRoute',
                            repeat: 'Repeat',
                            maxPlus: 'SuctionLevel' // MaxPlus is part of SuctionLevel
                        };

                        const mappingKey = mappingLookup[optionType];
                        if (!mappingKey) {
                            console.error('Unknown optionType: ' + optionType);
                            return deviceValue?.toString();
                        }

                        const mapping = DeviceStateMapping[mappingKey];

                        // Special handling for suction power (Max/MaxPlus)
                        if (mappingKey === 'SuctionLevel' && deviceValue === 3) {
                            const maxPlus = await getState(DEVICE_STATES.maxPlus);
                            return maxPlus ? "MaxPlus" : "Max";
                        }

                        // Normal mapping
                        const result = Object.keys(mapping).find(key => mapping[key] === deviceValue);
                        return result || deviceValue?.toString();
                    }

                    // Updates physical device states
                    async function updateDeviceStates(optionType, uiValue) {
                        ...
                        const deviceValue = getDeviceValue(deviceParam, uiValue);
                        if (deviceValue === null) return;

                        try {
                            // Special handling for suction strength
                            if (normalizedType === 'suction') {
                                await handleSuctionSettings(uiValue);
                            } else {
                                await setStateAsync(DEVICE_STATES[normalizedType], deviceValue);
                            }
                            console.log('[Sync] ' + normalizedType + ' = ' + uiValue + ' --> Device = ' + deviceValue);
                        } catch (err) {
                    ...
                    // Handles option button clicks with sync logic
                    async function handleOptionClick(e) {
                        ...
                        // Skip if no change
                        if (VisCleaningState[optionType] === value) return;
                        ...
                        try {
                            ...
                            // 3. Sync to device
                            await updateDeviceStates(optionType, value);
                    ...
                    // Fetches all device states and updates UI
                    async function refreshAllStates() {
                        if (isSyncing) return;
                        isSyncing = true;

                        try {
                            // 1. Load all device states
                            const deviceStates = {
                                mode: await getState(DEVICE_STATES.mode),
                                suction: await getState(DEVICE_STATES.suction),
                                maxPlus: await getState(DEVICE_STATES.maxPlus),
                                water: await getState(DEVICE_STATES.water),
                                route: await getState(DEVICE_STATES.route),
                                repeat: await getState(DEVICE_STATES.repeat)
                            };

                            let isUnchanged = true;

                            // 2. Process mode first
                            if (deviceStates.mode !== null) {
                                const uiMode = getUIModeFromDeviceValue(deviceStates.mode);
                                if (uiMode !== VisCleaningState.mode) {
                                    VisCleaningState.mode = uiMode;
                                    isUnchanged = false;
                                }
                            }

                            // 3. Process other settings
                            if (deviceStates.suction !== null) {
                                const newSuction = deviceStates.maxPlus ? "MaxPlus" :
                                    deviceStates.suction === 3 ? "Max" :
                                    await getUIValue('suction', deviceStates.suction);
                                if (newSuction !== VisCleaningState.suction) {
                                    VisCleaningState.suction = newSuction;
                                    isUnchanged = false;
                                }
                            }

                            if (deviceStates.water !== null) {
                                const newWater = await getUIValue('water', deviceStates.water);
                                if (newWater !== VisCleaningState.water) {
                                    VisCleaningState.water = newWater;
                                    isUnchanged = false;
                                }
                            }

                            if (deviceStates.route !== null) {
                                const newRoute = await getUIValue('route', deviceStates.route);
                                if (newRoute !== VisCleaningState.route) {
                                    VisCleaningState.route = newRoute;
                                    isUnchanged = false;
                                }
                            }

                            if (deviceStates.repeat !== null) {
                                const newRepeat = await getUIValue('repeat', deviceStates.repeat);
                                if (newRepeat !== VisCleaningState.repeat) {
                                    VisCleaningState.repeat = newRepeat;
                                    isUnchanged = false;
                                }
                            }
                            ...
                            // Update the states
                            VisCleaningState.Suction = VisCleaningState.suction;
                            VisCleaningState.Water = VisCleaningState.water;
                            VisCleaningState.Route = VisCleaningState.route;
                            VisCleaningState.Repeat = VisCleaningState.repeat;

                            console.log("Final cleaned state:", VisCleaningState);

                            // Call switchCleaningMode only if mode has changed
                            if (!isUnchanged) {
                                switchCleaningMode(VisCleaningState.mode, true); // true = isFromDeviceUpdate
                            }
                    ...
                    // Updates the UI based on device state changes
                    async function updateUIFromDeviceState(optionType, deviceValue) {
                        if (deviceValue == null) return;

                        const uiValue = await getUIValue(optionType, deviceValue);

                        if (uiValue && VisCleaningState[optionType] !== uiValue) {
                            VisCleaningState[optionType] = uiValue;

                            // Update lastSettings for current mode
                            if (VisCleaningModes[VisCleaningState.mode]) {
                                VisCleaningModes[VisCleaningState.mode].lastSettings[optionType] = uiValue;
                            }
                        }
                    }

                    // Convert numeric device value to UI mode name
                    function getUIModeFromDeviceValue(deviceValue) {
                        const mapping = DeviceStateMapping.CleaningMode;

                        // Search the mapping for the numeric value
                        const modeEntry = Object.entries(mapping).find(([key, value]) => value === deviceValue);

                        if (!modeEntry) {
                            console.warn('Unknown device mode value:', deviceValue);
                            return null;
                        }

                        const [uiKey] = modeEntry;

                        // Find the corresponding key in VisCleaningModes
                        const validModeKey = Object.keys(VisCleaningModes).find(key =>
                            key === uiKey || VisCleaningModes[key].name === uiKey
                        );

                        return validModeKey || null;
                    }
                    ...
                    function switchCleaningMode(newMode, isFromDeviceUpdate = false) {
                        ...
                        if (isSyncing && !isFromDeviceUpdate) {
                        ...
                        // Update device mode
                        if (!isFromDeviceUpdate) {
                            updateDeviceStates('mode', newMode);
                        }
                    ...
                    function updateButtonStates() {
                        // 1. Update options buttons (Suction/Water/etc.)
                        ...
                        // 2. Update mode selection buttons
                        document.querySelectorAll('[data-mode]').forEach(btn => {
                            const isActiveMode = VisCleaningState.mode === btn.dataset.mode;
                            btn.classList.toggle('active', isActiveMode);
                        });
                    }

                    // Special handling for suction strength settings
					function handleSuctionSettings(uiValue) {
                        const SETTINGS = {
                            delay: 2000,
                            maxRetries: 3
                        };
                        ...
                        for (let attempt = 1; attempt <= SETTINGS.maxRetries; attempt++) {
                            try {
                                const level = getDeviceValue('SuctionLevel', uiValue);
                                const isMaxPlus = uiValue === 'MaxPlus';

                                if (isMaxPlus) {
                                    setStateAsync('${prefix}.control.SuctionLevel', level);
                                    delay(SETTINGS.delay);
                                    setStateAsync('${prefix}.control.SuctionMaxPlus', 1);
                                } else {
                                    setStateAsync('${prefix}.control.SuctionMaxPlus', 0);
                                    delay(SETTINGS.delay);
                                    setStateAsync('${prefix}.control.SuctionLevel', level);
                                }

                                console.log('Successfully set suction to ' + uiValue);
                                return; // Success - exit loop

                            } catch (err) {
                                if (attempt === SETTINGS.maxRetries) {
                                    console.error('Failed after ' + SETTINGS.maxRetries + ' attempts: ' + err);
                                    throw err;
                                }

                                delay(SETTINGS.delay * attempt); // Exponential backoff
                            }
                        }
                    }

                    // Update the summary display with current settings
                    function updateSummaryDisplay() {
						const modeName = translations[UserLang]?.[VisCleaningModes[VisCleaningState.mode]?.name ||
							'Not selected'] ||
							VisCleaningModes[VisCleaningState.mode]?.name ||
						    'Not selected';
                        document.getElementById('mode-summary').textContent = modeName;
                        document.getElementById('suction-summary').textContent = getTranslatedValue(VisCleaningState.Suction);
                        document.getElementById('water-summary').textContent = getTranslatedValue(VisCleaningState.Water);
                        document.getElementById('route-summary').textContent = getTranslatedValue(VisCleaningState.Route);
                        document.getElementById('repeat-summary').textContent = getTranslatedValue(VisCleaningState.Repeat);
					...
					function getTranslatedValue(value) {
                        if (!value || value === "-") return translations[UserLang].summaryNotSet;
                        return translations[UserLang]["summary" + value] || value;
					}
                    ...
                    // Async wrapper for setState function
                    function setStateAsync(id, value) {
                        return new Promise((resolve, reject) => {
                    ...
                    // Initialize cleaning UI by creating all necessary elements
                    function initializeCleaningUI() {
                        const container = document.querySelector('.settings-container');
                        container.innerHTML = '';

                        // Create option groups for each setting type
                        Object.keys(VisCleaningOptions).forEach(optionType => {
                            ...
                            title.textContent = translations[UserLang][optionType.toLowerCase() + 'Label'].toUpperCase() || optionType.toUpperCase();
                        ...
                        // Initialize with current mode (either from device or default)
                        switchCleaningMode(VisCleaningState.mode || "Sweeping");
                    }

                    // Initialize state tracking for real-time updates
                    function initStateTracking() {
                        // Fallback: Periodic refresh (optional)
                        setInterval(refreshAllStates, 2000);
                        isSyncing = false;
                    }

                    // Initialize the complete cleaning menu
                    async function initCleaningMenu() {
                        try {
                            // 1. First try to load device states
                            await refreshAllStates();

                            // 2. Only proceed if we successfully got a mode
                            if (VisCleaningState.mode) {
                                ...
                                document.querySelectorAll('[data-mode]').forEach(btn => {
                                    ...
                                    // Mark active button based on loaded mode
                                    if (btn.dataset.mode === VisCleaningState.mode) {
                                ...
                                console.warn('Device state loaded but no mode was set');
                            }

                        } catch (err) {
                            console.error('Initialization failed:', err);
                            // Fallback to default mode if loading fails
                            if (!VisCleaningState.mode) {
                                switchCleaningMode("Sweeping");
                            }
                        }
                    }
    ...
    return `
                    ...
                    function handleClick(event) {
						...
						//console.log("Array settings for room and carpet colors:", colorMappings);
						//console.log("Original color query (hex):", hex);
                    ...
                    function formatDate(date) {
                        if (!date) return translations[UserLang].never;
                    ...
                    const labelManager = {
                     ...
                     createLabelContent: function(roomId, roomInfo, settings, stats) {
                        ...
                        return '<div class="label-content" ' + dataAttributes + '>' +
                          ...
                          '<div class="progress-container">' +
                            ...
                            '</div>' +
                            '<div class="progress-text">' + stats.CleanedArea + ' m² ' + stats.CoveragePercent + '% ' + translations[UserLang].covered + '</div>' +
                     ...
                     createIconItem: function(title, value, index, iconSvg) {
                      ...
                      updateLabelContent: function(roomId, stats, shouldAnimate = true) {
                        ...
                        if (progressText) {
                          progressText.textContent = stats.CleanedArea + ' m² ' + stats.CoveragePercent + '% ' + translations[UserLang].covered;
                        }
                        ...
                        const AnimationMaster = {
                            ...
                            toggle: function(event) {
                                if (AnimationMaster.running) {
                                    ...
                                    event.target.textContent = translations[UserLang].startMapSpectacleLabel; //'Start Map Spectacle';
                                } else {
                                    ...
                                    event.target.textContent = translations[UserLang].stopMapSpectacleLabel; //'Stop Map Spectacle';
                                }
                            }
                        };
                    ...
                    document.getElementById('Base-station-toggle').addEventListener('click', function() {
                        ...
                        if (stopStatusLoop) {
                            ...
                            button.textContent = translations[UserLang].stopBaseSpectacleLabel; //'Stop Base Spectacle';
                            ...
                            button.textContent = translations[UserLang].startBaseSpectacleLabel; //'Start Base Spectacle';
                    ...
                    const statusTranslations = {
                      'EN': {
						...
						'returning-to-change-mop': 'Returning to change mop'
                      },
                      'DE': {
                        'sweeping': 'Staubsaugen',
                        'mopping': 'Wischen',
						...
						'returning-to-change-mop': 'Zurück zum Mopp-Wechsel'
                      }
                    };
...
//status = 'drying';
    ...
    return '<div class="particle" style="--i:' + i + '"></div>';
                        		...
                        		case 'returning-to-change-mop':
                        		    effectsHTML = \`
                        		        <div class="status-effect change-mop-return-effect">
                        		            <div class="mop-change-swirl"></div>
                        		            <div class="mop-swap-animation">
                        		                <svg class="mop-icon mop-out" viewBox="0 0 24 24">
                        		                    <path fill="white" d="M15.5 14.5c0-2.8 2.2-5 5-5 .36 0 .71.04 1.05.11L23.64 7c-.45-.34-4.93-4-11.64-4C5.28 3 .81 6.66.36 7L12 21.5l3.5-4.36V14.5z"/>
                        		                </svg>
                        		                <svg class="mop-icon mop-in" viewBox="0 0 24 24">
                        		                    <path fill="white" d="M15.5 14.5c0-2.8 2.2-5 5-5 .36 0 .71.04 1.05.11L23.64 7c-.45-.34-4.93-4-11.64-4C5.28 3 .81 6.66.36 7L12 21.5l3.5-4.36V14.5z"/>
                        		                </svg>
                        		            </div>
                        		    ...
                        		    effectsCSS = \`
                        		        ...
                        		        .mop-change-swirl {
                        		            ...
                        		            animation: swirl-spin-change 2s linear infinite;
                        		            border-radius: 50%;
                        		        }
                        		        .mop-swap-animation {
                        		            position: absolute;
                        		        ...
                        		        .mop-out {
                        		            left: 20%;
                        		            animation: mop-out-animation 2s infinite;
                        		        }
                        		        .mop-in {
                        		            left: 40%;
                        		            animation: mop-in-animation 2s infinite;
                        		        }
                        		        .change-arrow {
                        		            position: absolute;
                        		            width: 40%;
                        		            ...
                        		            background: white;
                        		            clip-path: polygon(0 0, 80% 0, 100% 50%, 80% 100%, 0 100%);
                        		            animation: change-arrow-move 1.5s infinite;
                        		        }
                        		        @keyframes swirl-spin-change {
                        		            from { transform: rotate(0deg); }
                        		            to { transform: rotate(360deg); }
                        		        }
                        		        @keyframes mop-out-animation {
                        		            0%, 100% { opacity: 0.7; transform: translateX(0); }
                        		            50% { opacity: 0.3; transform: translateX(-10px); }
                        		        }
                        		        @keyframes mop-in-animation {
                        		            0%, 100% { opacity: 0.3; transform: translateX(0); }
                        		            50% { opacity: 0.7; transform: translateX(10px); }
                        		        }
                        		        @keyframes change-arrow-move {
                        		            0% { transform: translateX(0); opacity: 0; }
                        		            30%, 70% { opacity: 1; }
                        		            100% { transform: translateX(20px); opacity: 0; }
                        		        }\`;
                    ...
                    function initAllInteractions() {
                        ...
                        initRobotControl();
											...
											// After the page is loaded: draw carpets on the color map
    console.log('Initializing color map with carpets...');

    // Draw carpets on the color map
    ${Object.entries(elements.carpets).map(([id, rect]) => {
    const color = ColorsCarpet[id % ColorsCarpet.length];

    if (Array.isArray(rect) && rect.length === 3 && Array.isArray(rect[0])) {
      // New polygon structure
      const [polygonPoints, material, flags] = rect;
      const points = [];
      for (let i = 0; i < polygonPoints.length; i += 2) {
        const [tx, ty] = toCanvas(polygonPoints[i], polygonPoints[i + 1]);
      ...
      return `
            // Add carpet ${id} to the color map (polygon)
            colorMapCtx.fillStyle = '${color}';
            colorMapCtx.beginPath();
            colorMapCtx.moveTo(${points[0][0]}, ${points[0][1]});
            ${points.slice(1).map(point => `colorMapCtx.lineTo(${point[0]}, ${point[1]});`).join('')}
            colorMapCtx.closePath();
            colorMapCtx.fill();`;
    } else {
      // Old rectangle structure
      const [x1, y1, x2, y2] = rect;
      const [tx1, ty1] = toCanvas(x1, y1);
      const [tx2, ty2] = toCanvas(x2, y1);
      const [tx3, ty3] = toCanvas(x2, y2);
      ...
      return `
            // Add carpet ${id} to the color map (rectangle)
            colorMapCtx.fillStyle = '${color}';
            colorMapCtx.beginPath();
            colorMapCtx.moveTo(${tx1}, ${ty1});
            colorMapCtx.lineTo(${tx2}, ${ty2});
    ...
    console.log('Color map initialized with carpets');
