...
class LearningSystem {
  ...
  async updateRoomCoverage(roomName, coverage, cleanedArea) {
    ...
    // Abort if the learning system is disabled or if statistics have not been initialized yet.
      ...
      if (profile.coverageHistory.length > 0) {
        // Store only the current value, not the full historical data
        profile.coverageHistory[profile.coverageHistory.length - 1] = coverage;
      } else {
